...
const LoginScreen = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // ✅ استخدام Supabase Auth للتحقق الحقيقي
    ...
    if (error) {
      // فشل تسجيل الدخول: إيميل أو باسورد خطأ
      toast({
        title: t('error'),
    ...
    if (data?.user) {
      // نجاح تسجيل الدخول
      // تخزين بيانات المستخدم في localStorage (كما كان يفعل الكود القديم)
  ...
  const registrationPath =
    userType === 'customer' ? '/register/customer' : '/register/technician';
      ...
      <Card className="w-full max-w-md shadow-2xl glassmorphism-card">
        <CardHeader className="text-center">
          ...
          <CardDescription>
            {t(
              'loginSubtitle',
              `Please enter your credentials to access your ${userType} account.`
            )}
          </CardDescription>
        </CardHeader>
        <CardContent>
