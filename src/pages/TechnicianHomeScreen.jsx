...
const TechnicianHomeScreenContent = () => {
  ...
  useEffect(() => {
    const loadTech = async () => {
      ...
      if (error || !tech) {
        setErrorMsg('فشل تحميل بيانات الفني.');
        setIsLoading(false);
        return;
      }

      if (tech.status !== 'approved') {
        setErrorMsg(`حالة حسابك: ${tech.status}. لا يمكنك الدخول حتى يوافق الأدمن.`);
        setIsLoading(false);
        return;
      }
  ...
  // جلب الطلبات (بدون فلترة null للموقع)
  ...
  const handlePlaceBid = async (requestId) => {
    ...
    if (error) {
      ...
      toast({ description: 'تم تقديم العرض.' });
    }
  };

  const handleUpdateStatus = async (requestId, newStatus) => {
    await supabase.from('maintenance_requests').update({ status: newStatus }).eq('id', requestId);
    setMyAssignedRequests((prev) => prev.map((r) => (r.id === requestId ? { ...r, status: newStatus } : r)));
    toast({ description: `الحالة: ${newStatus}` });
  };

  const handleDelay = async (requestId) => {
    if (!delayReason.trim()) return;
    await supabase.from('maintenance_requests').update({ notes: delayReason, expires_at: new Date(Date.now() + 30 * 60000).toISOString() }).eq('id', requestId);
  ...
  if (errorMsg) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-4">
        ...
        <p className="text-center text-lg font-semibold mb-4">{errorMsg}</p>
        <Button onClick={async () => { await supabase.auth.signOut(); localStorage.clear(); navigate('/user-type'); }}>تسجيل الخروج</Button>
      </div>
    );
  }
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="min-h-screen p-4 md:p-8 bg-gradient-to-br from-primary/5 via-background to-accent/5">
      ...
      <div className="flex justify-end mb-4">
        <Button variant="outline" size="sm" onClick={fetchPendingRequests} disabled={isLoading}><RefreshCw size={16} className="mr-1" /> تحديث الطلبات</Button>
      </div>

      <section className="mb-8">
        <h2 className="text-2xl font-bold mb-4">طلبات قريبة</h2>
        {isLoading ? (
          <div className="text-center py-8"><RefreshCw className="animate-spin text-primary mx-auto mb-2" size={32} /><p>جاري تحميل الطلبات...</p></div>
              ...
              <div key={req.id} className="bg-card p-4 rounded-2xl shadow-md border hover:shadow-lg transition flex justify-between items-start">
                <div>
                  ...
                  <div className="flex gap-2 mt-2 text-xs">
                    {req.distance != null && (
                      <span className="bg-blue-50 text-blue-700 px-2 py-1 rounded-full"><MapPin size={12} className="inline" /> {req.distance.toFixed(1)} كم</span>
                    )}
                    <span className="bg-green-50 text-green-700 px-2 py-1 rounded-full"><Clock size={12} className="inline" /> {req.distance ? Math.round(req.distance / 30) * 60 : '؟'} دقيقة</span>
                  </div>
                  {req.address && <p className="text-xs text-muted-foreground mt-1">📍 {req.address}</p>}
                </div>
                <Button onClick={() => handlePlaceBid(req.id)} disabled={submittingBid === req.id}>تقديم عرض</Button>
              </div>
            ))}
          </div>
        )}
            ...
            <div key={req.id} className="bg-card p-4 rounded-2xl shadow mb-3 border-l-4 border-primary">
              <div className="flex flex-col md:flex-row justify-between gap-4">
                <div>
                  ...
                  {req.customer?.phone && <p className="text-xs flex items-center gap-1 mt-1"><Phone size={12} /> {req.customer.phone}</p>}
                  <p className="text-xs mt-1">الحالة: <span className="font-semibold">{req.status}</span></p>
                  {req.expires_at && <p className="text-xs text-red-500 flex items-center gap-1"><Clock size={12} /> {getTimeLeft(req.expires_at)}</p>}
                </div>
                <div className="flex flex-wrap gap-2 items-start">
                  ...
                  {req.status === 'on_the_way' && <Button size="sm" onClick={() => handleUpdateStatus(req.id, 'in_progress')}><Wrench size={14} className="mr-1" /> بدء الإصلاح</Button>}
                  {req.status === 'in_progress' && <Button size="sm" onClick={() => handleUpdateStatus(req.id, 'completed')}><CheckCircle size={14} className="mr-1" /> تم الإصلاح</Button>}
